...
Trabalhou-se com valores de $\alpha$ como escalar e como vetor em determinados momentos. Para o seu uso escalar fixou-se o valor $5$, ja para $\alpha$
...
df %>%
  kbl(caption = 'EMV em Diferentes Tamanhos Amostrais') %>%
...
df %>%
  kbl(caption = 'Poder em Diferentes Tamanhos Amostrais') %>%
  kable_classic(full_width = F, html_font = "Cambria")
```
